 ...
 * {@link ItemTouchCustomHelper.Callback#onMove(RecyclerView, ViewHolder, ViewHolder)} and / or
 * {@link ItemTouchCustomHelper.Callback#onSwiped(ViewHolder, int)}.
 * <p>
 ...
 * {@link ItemTouchCustomHelper.Callback} class or implementing {@link ItemTouchCustomHelper.ViewDropHandler}
 ...
 * overriding {@link ItemTouchCustomHelper.Callback#onChildDraw(Canvas, RecyclerView, ViewHolder, float, float, int,
 * boolean)}
 * or {@link ItemTouchCustomHelper.Callback#onChildDrawOver(Canvas, RecyclerView, ViewHolder, float, float, int,
 * boolean)}.
 * <p/>
 ...
 */
public class ItemTouchCustomHelper extends RecyclerView.ItemDecoration
     ...
     */
    ItemTouchCustomHelper.Callback mCallback;
     ...
     * {@link ItemTouchCustomHelper.Callback#getAbsoluteMovementFlags(RecyclerView, ViewHolder)} for the current
     * action state.
     */
    int mSelectedFlags;
     ...
     * jumpy UIs.
     */
    List<ItemTouchCustomHelper.RecoverAnimation> mRecoverAnimations = new ArrayList<ItemTouchCustomHelper.RecoverAnimation>();
    ...
    private final OnItemTouchListener mOnItemTouchListener
        ...
        public boolean onInterceptTouchEvent(RecyclerView recyclerView, MotionEvent event) {
            View view = findChildView(event);
            int position = recyclerView.getChildLayoutPosition(view);
            if(mCallback!=null && mCallback instanceof ItemTouchHelperCallback
                    && ((ItemTouchHelperCallback) mCallback).getSelectNoNeedSwipeList()!=null
                    && ((ItemTouchHelperCallback) mCallback).getSelectNoNeedSwipeList().contains(position)){
                return true;
            }
            ...
            if (action == MotionEvent.ACTION_DOWN) {
                ...
                if (mSelected == null) {
                    final ItemTouchCustomHelper.RecoverAnimation animation = findAnimation(event);
                    if (animation != null) {
     ...
     */
    public ItemTouchCustomHelper(Callback callback) {
        mCallback = callback;
    }
    ...
    private void destroyCallbacks() {
        ...
        for (int i = recoverAnimSize - 1; i >= 0; i--) {
            final ItemTouchCustomHelper.RecoverAnimation recoverAnimation = mRecoverAnimations.get(0);
    ...
    private void initGestureDetector() {
        ...
        mGestureDetector = new GestureDetectorCompat(mRecyclerView.getContext(),
                new ItemTouchCustomHelper.ItemTouchHelperGestureListener());
    }
    ...
    void select(ViewHolder selected, int actionState) {
        ...
        if (mSelected != null) {
            ...
            if (prevSelected.itemView.getParent() != null) {
                ...
                final ItemTouchCustomHelper.RecoverAnimation rv = new ItemTouchCustomHelper.RecoverAnimation(prevSelected, animationType,
    ...
    void postDispatchSwipe(final ItemTouchCustomHelper.RecoverAnimation anim, final int swipeDir) {
    ...
    int endRecoverAnimation(ViewHolder viewHolder, boolean override) {
        final int recoverAnimSize = mRecoverAnimations.size();
        for (int i = recoverAnimSize - 1; i >= 0; i--) {
            final ItemTouchCustomHelper.RecoverAnimation anim = mRecoverAnimations.get(i);
     ...
     * {@link ItemTouchCustomHelper.Callback#isLongPressDragEnabled()}.
     * <p>
     ...
     * ItemTouchHelper
     * is attached.</li>
     * <li>{@link ItemTouchCustomHelper.Callback} must have dragging enabled.</li>
     ...
     *                   RecyclerView.
     * @see ItemTouchCustomHelper.Callback#isItemViewSwipeEnabled()
     */
     ...
     * behavior
     * by overriding {@link ItemTouchCustomHelper.Callback}
     * <p>
     * For this method to work:
     * <ul>
     * <li>The provided ViewHolder must be a child of the RecyclerView to which this
     * ItemTouchHelper is attached.</li>
     * <li>{@link ItemTouchCustomHelper.Callback} must have swiping enabled.</li>
     * <li>There must be a previous touch event that was reported to the ItemTouchHelper
     * through RecyclerView's ItemTouchListener mechanism. As long as no other ItemTouchListener
     * grabs previous events, this should work as expected.</li>
     * </ul>
    ...
    ItemTouchCustomHelper.RecoverAnimation findAnimation(MotionEvent event) {
        ...
        if (Math.abs(mDx) > Math.abs(mDy)) {
            if ((swipeDir = checkHorizontalSwipe(viewHolder, flags)) > 0) {
                ...
                if ((originalFlags & swipeDir) == 0) {
                    // convert to relative
                    return ItemTouchCustomHelper.Callback.convertToRelativeDirection(swipeDir,
                            ViewCompat.getLayoutDirection(mRecyclerView));
                }
                return swipeDir;
            }
     ...
     * {@link ItemTouchCustomHelper}.
     */
    public static interface ViewDropHandler {

        /**
         * Called by the {@link ItemTouchCustomHelper} after a View is dropped over another View.
         * <p>
     ...
     * {@link ItemTouchCustomHelper.SimpleCallback}.
     * <p>
     * If user drags an item, ItemTouchHelper will call
     * {@link ItemTouchCustomHelper.Callback#onMove(RecyclerView, ViewHolder, ViewHolder)
    ...
    public abstract static class Callback {
        ...
        static {
            if (Build.VERSION.SDK_INT >= 21) {
                sUICallback = new ItemTouchCustomUIUtilImpl.Lollipop();
            } else if (Build.VERSION.SDK_INT >= 11) {
                sUICallback = new ItemTouchCustomUIUtilImpl.Honeycomb();
            } else {
                sUICallback = new ItemTouchCustomUIUtilImpl.Gingerbread();
            }
        }

        /**
         * Returns the {@link ItemTouchUIUtil} that is used by the {@link ItemTouchCustomHelper.Callback} class for
         * visual
         ...
         * <p>
         * By default, {@link ItemTouchCustomHelper.Callback} applies these changes on
         ...
         * </pre>
         *
         * @return The {@link ItemTouchUIUtil} instance that is used by the {@link ItemTouchCustomHelper.Callback}
         */
         ...
         *                    it is cleared.
         * @param actionState One of {@link ItemTouchCustomHelper#ACTION_STATE_IDLE},
         *                    {@link ItemTouchCustomHelper#ACTION_STATE_SWIPE} or
         *                    {@link ItemTouchCustomHelper#ACTION_STATE_DRAG}.
        ...
        public void onMoved(final RecyclerView recyclerView,
            ...
            if (layoutManager instanceof ItemTouchCustomHelper.ViewDropHandler) {
                ((ItemTouchCustomHelper.ViewDropHandler) layoutManager).prepareForDrop(viewHolder.itemView,
                        target.itemView, x, y);
                return;
            }
        ...
        void onDraw(Canvas c, RecyclerView parent, ViewHolder selected,
                    List<ItemTouchCustomHelper.RecoverAnimation> recoverAnimationList,
                    int actionState, float dX, float dY) {
            final int recoverAnimSize = recoverAnimationList.size();
            for (int i = 0; i < recoverAnimSize; i++) {
                final ItemTouchCustomHelper.RecoverAnimation anim = recoverAnimationList.get(i);
                anim.update();
     ...
     * </pre>
     */
    public abstract static class SimpleCallback extends ItemTouchCustomHelper.Callback {
